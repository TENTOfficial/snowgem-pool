Thanks joshuayabut for the pool.
...
sudo n stable
git clone https://github.com/Snowgem/snowgem-pool.git z-nomp
cd z-nomp
npm update
npm install
```
...
##### Pool config
Take a look at the example json file inside the `pool_configs` directory. Rename it to `snowgem.json` and change the
...
```
Example: inside `snowgem.conf` add the line
```
